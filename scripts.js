...
const svg = document.querySelector('svg');
    ...
    window.addEventListener('scroll', () => {
        ...
        const zoomScale = 1 + (scrollTop / maxScroll) * 15;
        svg.style.transform = `scale(${zoomScale})`;

        // Change background color to black when 90% scrolled
        if (scrollTop / maxScroll > 0.9) {
            body.classList.add('black-theme');  // Add black-theme class
            svg.style.opacity = '0';  // Fade out the SVG
        } else {
            body.classList.remove('black-theme');  // Remove black-theme class
            svg.style.opacity = '1';  // Reset SVG visibility
        }
    });  